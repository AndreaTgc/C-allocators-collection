 ...
 * STB-style implementation of a fixed size arena allocator, it currently uses the standard malloc
 * and free functions to handle the allocation and deletion of the arena itself, this may change in 
 * the future once i implement my own functions through syscalls.
 *
 * @note This library comes with an extremely simple API, here's a brief introduction to what you'll
 * be able to do with the following code (the code is also commented in case you have any doubts)
 *
 * 'arena_init' -> Allocates a new arena on the heap and returnst the pointer to it. You need to pass
 * the number of bytes as a parameter for the allocation. Please note that, in case the allocation fails,
 * the function will return NULL (the user is heavily encouraged to handle this case in their code)
 *
 * 'arena_alloc' -> Reserves the amount of bytes passed as a parameter and returns the pointer to it.
 * Please note that you won't be able to exceed the number of bytes specified in the 'arena_init' call.
 * Example: the user allocates an arena by calling 'arena_init(10)' and then calls 'arena_alloc(11)'.
 * The 'arena_alloc' function call will return NULL because the user is trying to exceed the number of bytes
 * preiviously specified.
 *
 * 'arena_reset' -> This function is extremely straight forward, it sets arena->size (basically the allocation
 * counter) to 0.
 *
